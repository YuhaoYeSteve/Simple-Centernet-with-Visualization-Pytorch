...
class FocalLoss(nn.Module):
    ...
    def forward(self, out, target):
        if self.config.if_debug:
            return self.neg_loss(out, target, vis=self.config.vis)
        else:
...
class NormRegL1Loss(nn.Module):
    ...
    def forward(self, output, mask, ind, target, _transpose_and_gather_feat=None):
        pred = _transpose_and_gather_feat(output, ind)
        mask = mask.unsqueeze(2).expand_as(pred).float()
        # loss = F.l1_loss(pred * mask, target * mask, reduction='elementwise_mean')
...
class CtdetLoss(torch.nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.crit = FocalLoss(self.config)
        self.crit_reg = RegL1Loss(self.config)
        self.crit_wh = RegL1Loss(self.config)
